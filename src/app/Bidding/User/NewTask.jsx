...
export default function BiddingNewTask() {
  ...
  useEffect(() => {
    if (
      selectedAddressObj &&
      isAddressConfirmed[
        profile?.full_address?.indexOf(selectedAddressObj)
      ]
    ) {
      updateLocation(selectedAddressObj);
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-start bg-gray-50 px-4 py-8">
        <div className="bg-white rounded-2xl p-6 sm:p-8 w-full max-w-lg sm:max-w-xl lg:max-w-3xl shadow">
          <form onSubmit={handleSubmit} className="w-full space-y-6">
            ...
            {/* Deadline */}
            <div>
              <label className="block mb-1 font-medium">Add Deadline</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <ReactDatePicker
                  ...
                  dateFormat="yyyy-MM-dd HH:mm"
                  placeholderText="Select deadline"
                  className="w-full border-2 border-gray-400 rounded-lg pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
            </div>
            ...
            <div className="border-2 border-gray-400 rounded-xl p-6 text-center">
              <label className="cursor-pointer flex flex-col items-center">
                ...
                />
                <button
                  className="px-4 py-2 border border-green-600 rounded-md mt-3 text-green-600 font-medium"
                  onClick={(e) => {
                    e.preventDefault();
                    document.querySelector('input[type="file"]').click();
                  }}
              ...
              <span className="text-xs text-green-600 font-medium">
                Upload (.png, .jpg, .jpeg) Files - Max 5 photos (Optional)
              </span>
            </div>

            <button className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-xl mt-4">
              Post Task
            </button>
          </form>
        </div>
