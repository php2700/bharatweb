...
export default function Accepted({
  ...
  if (!serviceProvider && !assignedWorker) return null;
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const totalAmount = parseFloat(amount) + parseFloat(amount) * 0.09; // amount + tax
      ...
      if (response.status === 200) return response.data.razorOrder;
  ...
  const addPaymentMethod = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || "Failed to add COD payment stage");
  ...
  const projectAmount = parseFloat(fullPaymentHistory.total_expected || 0);

  const payToApp = parseFloat(fullPaymentHistory.amount || 0);

  // const platformFees = (payToApp * 0.1).toFixed(2); // 10% fee

  const platformFees = 220;

  const remainingAmount = (projectAmount - payToApp).toFixed(2);

  const totalToRelease = (payToApp - platformFees).toFixed(2);
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-6 max-w-4xl">
      ...
      {/* ✅ Existing Tables and Functionality ... (unchanged) */}

      {/* [Your existing worker display + payment history code remains unchanged above] */}
      ...
      {/* ✅ New Payment Breakdown Table */}

      <div className="p-4 bg-white shadow-md rounded-lg mt-8">

        <h3 className="text-lg font-semibold mb-3 text-[#228B22]">Payment Breakdown</h3>
        ...
        <table className="w-full border border-gray-300 rounded-md overflow-hidden">
          ...
          <thead style={{ backgroundColor: "#228B22", color: "white" }}>
            ...
            <tr>
              ...
              <th className="border p-2 text-left">Project Amount</th>

              <th className="border p-2 text-left">Pay to App</th>

              <th className="border p-2 text-left">Platform Fees (10%)</th>

              <th className="border p-2 text-left">Remaining Amount</th>

              <th className="border p-2 text-left">Total Amount to Be Released</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="border p-2">₹{projectAmount}</td>

              <td className="border p-2">₹{payToApp}</td>

              <td className="border p-2">₹{platformFees}</td>

              <td className="border p-2">₹{remainingAmount}</td>

              <td className="border p-2 font-semibold text-[#228B22]">₹{totalToRelease}</td>
