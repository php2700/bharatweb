...
export default function ViewProfile() {
  ...
  const [orderData, setOrderData] = useState(null);
  const [assignedWorker, setAssignedWorker] = useState(null);
  const [showCancelModal, setShowCancelModal] = useState(false); // Renamed for clarity
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [serviceProviders, setServiceProviders] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  ...
  const [bannerLoading, setBannerLoading] = useState(true);
  const [bannerError, setBannerError] = useState(null);
  const [showRefundButton, setShowRefundButton] = useState(false); // New state for refund button visibility

  const token = localStorage.getItem("bharat_token"); // Define token here to use in fetchBannerImages
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [id, orderData?.hire_status, token]); // Added token to dependency array
  ...
  // Modified handleConfirmCancel
  const handleConfirmCancel = async () => {
    setShowCancelModal(false); // Close the initial cancel confirmation modal
    try {
      await axios.post(
        `${BASE_URL}/emergency-order/cancel`,
        {
      ...
      // After successful cancellation, show the refund button
      setShowRefundButton(true);
      Swal.fire({
        icon: "success",
        title: "Order Cancelled!",
        text: "Your order has been cancelled. You can now request a refund.",
        confirmButtonColor: "#228B22",
      }).then(() => {
        // Optionally update order status in UI without refetching all data
        setOrderData(prevData => ({ ...prevData, hire_status: "cancelled" }));
      });
    } catch (err) {
      setError("Failed to cancel order. Please try again later.");
      Swal.fire({
  ...
  // New function for Refund Request
  // const handleRefundRequest = async () => {
  //   try {
  //           console.log("Refund API URL:", `${BASE_URL}/user/requestRefundEmergencyOrder`);
  //     console.log("Refund order_id:", id);
  //     // Assuming a new API endpoint for refund requests
  //     const response = await axios.post(
  //       `${BASE_URL}/user/requestRefundEmergencyOrder`, // Replace with your actual refund API endpoint
  //       {
  //         orderId: id,
  //         reason : "service not compelete"
  //         // You might send other relevant data like user_id, amount, etc.
  //         // The API should default the status to 'pending'
  //       },
  //       {
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  //         confirmButtonColor: "#228B22",
  //       }).then(() => {
  //         setShowRefundButton(false); // Hide the refund button after request
  //         // Optionally update order data to reflect refund status, if your order data includes it.
  ...
  //         icon: "error",
  //         title: "Oops!",
  //         text: response.data.message || "Failed to submit refund request. Please try again.",
  //         confirmButtonColor: "#FF0000",
  //       });
  //     }
  //   } catch (err) {
  ...
  //       icon: "error",
  //       title: "Oops!",
  //       text: "An error occurred while submitting your refund request. Please try again.",
  //       confirmButtonColor: "#FF0000",
  //     });
  //   }
  // };
  ...
  try {
    const token = localStorage.getItem("bharat_token");
    const response = await axios.post(
      `${BASE_URL}/user/requestRefundEmergencyOrder`,
      { orderId :id,
         reason :"serv not cmp"},
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  return (
    <>
      ...
      ${orderData?.hire_status === "pending" ? "bg-yellow-500" : ""}
      ${orderData?.hire_status === "cancelled" ? "bg-[#FF0000]" : ""}
      ${orderData?.hire_status === "refunded" ? "bg-purple-500" : ""} {/* New status for refunded */}
      ${orderData?.hire_status === "completed" ? "bg-[#228B22]" : ""}
      ${orderData?.hire_status === "cancelldispute" ? "bg-[#FF0000]" : ""}
      ${orderData?.hire_status === "assigned" ? "bg-blue-500" : ""}`}
                  >
            ...
            <div className="text-center mb-6">
              {orderData?.hire_status === "cancelled" && !showRefundButton ? (
                 <span className="px-8 py-2 bg-[#FF0000] text-white rounded-lg text-lg font-semibold">
                   Cancelled by User
                 </span>
              ) : orderData?.hire_status === "completed" ? (
              ...
              ) : orderData?.hire_status !== "assigned" && orderData?.hire_status !== "refunded" && !showRefundButton ? (
                <button
                  className="px-8 py-3 bg-[#FF0000] text-white rounded-lg text-lg font-semibold hover:bg-red-700"
                  onClick={() => setShowCancelModal(true)} // Use showCancelModal
                >
                  Cancel Task
                </button>
              ) : null}

              {showRefundButton && orderData?.hire_status === "cancelled" && (
                <button
                  className="px-8 py-3 bg-blue-600 text-white rounded-lg text-lg font-semibold hover:bg-blue-700 ml-4"
      ...
      <Footer />
      {showCancelModal && ( // Use showCancelModal
        <div
          className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center"
          role="dialog"
          aria-labelledby="modal-title"
