...
const Chat = () => {
  ...
  return (
    <>
      ...
      <div
  className="flex bg-gray-100 mt-[80px] min-h-[calc(100vh-80px)] overflow-hidden"
  style={{ height: "calc(100vh - 80px)" }}
>
  {/* Sidebar */}
  <div
    /* Sidebar: hidden on small screens when a chat is open, visible on md+ always.
       This uses the existing currentChat state only for UI — no new logic. */
    className={`${currentChat ? "hidden md:flex" : "flex"} w-80 md:w-96 bg-white border-r border-gray-200 flex-col h-full z-10`}
  >
          ...
          >
            ...
            <div className="ml-4 flex-1 min-w-0">
              <div className="flex justify-between items-baseline">
                ...
                <h3
                  className={`text-sm font-semibold truncate capitalize ${isActive ? "text-[#228B22]" : "text-gray-900"}`}
                >
                  {otherUser?.full_name || "User"}
                </h3>
              ...
              <div className="flex justify-between items-center mt-1">
                <p
                  className={`text-sm truncate w-4/5 ${conv.unreadCount > 0 ? "font-semibold text-gray-800" : "text-gray-500"}`}
                >
  ...
  {/* Chat Area */}
  <div
    /* Chat area: hidden on small screens when no chat is opened, visible when currentChat exists.
       On md+ always visible. */
    className={`${currentChat ? "flex" : "hidden md:flex"} flex-1 flex flex-col bg-[#F3F4F6] relative h-full w-full`}
  >
    {currentChat ? (
      <>
        {/* Chat Header */}
        <div className="h-16 bg-white border-b border-gray-200 flex items-center px-4 md:px-6 shadow-sm flex-shrink-0 z-30 sticky top-0">
          {/* Mobile Back Button */}
          <button
            onClick={() => setCurrentChat(null)}
            className="md:hidden mr-3 flex items-center gap-1 text-[#228B22] font-semibold"
          ...
          <div className="flex items-center gap-3">
            <img
              src={currentChat.members.find((m) => m._id !== senderId)?.profile_pic || defaultPic}
              className="w-10 h-10 rounded-full object-cover"
              alt="Current User"
            />
            <div>
              <h3 className="font-bold text-gray-800">
                {(currentChat.members.find((m) => m._id !== senderId)?.full_name || "Chat").replace(/^\w/, (c) => c.toUpperCase())}
              </h3>
              {/* Check online status */}
        ...
        {/* Messages List */}
        {/* Only this area scrolls (no double scroll). padding bottom ensures input doesn't overlay last message. */}
        <div className="flex-1 overflow-auto p-6 space-y-4 custom-scrollbar bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat bg-opacity-5">
          {isLoading ? (
                ...
                >
                  <div
                    className={`relative max-w-[70%] lg:max-w-[50%] p-3 shadow-sm rounded-2xl
                      ${isMe ? "bg-[#228B22] text-white rounded-tr-none" : "bg-white text-gray-800 rounded-tl-none border border-gray-100"}`}
                  >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 text-gray-400">
              <label className="p-2 hover:bg-gray-100 rounded-full cursor-pointer transition">
                <input type="file" accept="image/*" multiple onChange={handleFileChange} className="hidden" disabled={isLoading || !isChatInitialized} />
                 <FiImage size={20} /> 
              </label>
              <label className="p-2 hover:bg-gray-100 rounded-full cursor-pointer transition">
                <input type="file" accept=".pdf,.doc,.docx" multiple onChange={handleFileChange} ref={fileInputRef} className="hidden" disabled={isLoading || !isChatInitialized} />
                 <FiPaperclip size={20} /> 
              </label>
            </div>
            */}
