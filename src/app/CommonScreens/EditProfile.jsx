...
export default function EditProfile() {
  ...
  const buildAddressOptions = () => {
    const opts = [];
    if (Array.isArray(profile?.full_address)) {
      profile.full_address.forEach((addr, i) => {
        opts.push({
          value: `full_${i}`,
          label: `${addr.title || "Address"} – ${addr.address}`,
          lat: addr.latitude,
          lng: addr.longitude,
        });
      });
    ...
    const shop = profile?.businessAddress;
    if (shop?.address) {
      const already = opts.some((o) => o.lat === shop.latitude && o.lng === shop.longitude);
      if (!already) {
        opts.push({
          value: "shop",
          label: `Shop – ${shop.address}`,
          lat: shop.latitude,
          lng: shop.longitude,
        });
      }
  ...
  const getDocErrors = () => {
    ...
    if (activeTab === "worker") {
      ...
      const partialAadhaar = (documents.aadhaarFront.preview || documents.aadhaarBack.preview) && !hasAadhaar;
      if (!hasAadhaar) {
      ...
      const partialPan = (documents.panFront.preview || documents.panBack.preview) && !hasPan;
      if (!hasPan) {
        errs.pan = partialPan
  ...
  const handleDocUpload = (e, type) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      Swal.fire({ icon: "error", title: "Invalid File", text: "Only JPEG, PNG, AVIF, or GIF images are allowed." });
      return;
    }
    if (file.size > 20 * 1024 * 1024) {
      Swal.fire({ icon: "error", title: "File Too Large", text: "File size must be under 20 MB." });
      return;
    }
  ...
  const handleDocDelete = async (type) => {
    ...
    try {
      ...
      console.error(err);
      Swal.fire({ icon: "error", title: "Failed", text: err.message || "Could not delete image" });
    }
  };
    ...
    const processDocuments = async () => {
        ...
        if (doc && Array.isArray(doc.images)) {
          for (const img of doc.images) {
            if (typeof img === "string" && img.trim() && !seenUrls.has(img)) {
              const ok = await isValidImageUrl(img);
              if (ok) {
                seenUrls.add(img);
                ...
                } else if (doc.documentName === "AadharCard") {
                  if (!temp_docs.aadhaarFront.preview) temp_docs.aadhaarFront = { preview: img, file: null, path };
                  else if (!temp_docs.aadhaarBack.preview) temp_docs.aadhaarBack = { preview: img, file: null, path };
                } else if (doc.documentName === "PanCard") {
                  if (!temp_docs.panFront.preview) temp_docs.panFront = { preview: img, file: null, path };
                  else if (!temp_docs.panBack.preview) temp_docs.panBack = { preview: img, file: null, path };
                } else if (doc.documentName === "selfAttached") {
                  if (!temp_docs.selfie.preview) temp_docs.selfie = { preview: img, file: null, path };
                }
              } else {
                inaccessible.push({ docName: doc.documentName, url: img });
              }
            }
          }
        }
      ...
      if (inaccessible.length) {
        Swal.fire({
          ...
          text: `Some images could not be loaded. Please re-upload: ${[...new Set(inaccessible.map(i => i.docName))].join(", ")}`,
        });
      }
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        Swal.fire({ icon: "error", title: "Failed", text: "Failed to fetch categories" });
      }
    };
    fetchCategories();
  }, []);
  ...
  const handleCategoryChange = async (selected) => {
    ...
    setFormData(prev => ({
      ...
      emergencysubcategory: catId !== prev.category ? [] : prev.emergencysubcategory,
    }));
    setErrors(prev => ({ ...prev, category: catId ? undefined : "Category is required." }));

    if (!catId) return;

    try {
      const token = localStorage.getItem("bharat_token");
      const [subRes, emerRes] = await Promise.all([
        fetch(`${BASE_URL}/subcategories/${catId}`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${BASE_URL}/emergency/subcategories/${catId}`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);

      const [subData, emerData] = await Promise.all([subRes.json(), emerRes.json()]);

      if (subRes.ok) {
        setSubcategories((subData.data || []).map(s => ({ value: s._id, label: s.name })));
      }
      if (emerRes.ok) {
        setEmergencysubcategories((emerData.data || []).map(e => ({ value: e._id, label: e.name })));
      }
    } catch (err) {
      Swal.fire({ icon: "error", title: "Failed", text: "Failed to fetch sub-categories" });
    }
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "name") {
      ...
      const err = !clean.trim() ? "Name is required." :
        clean.trim().length < 3 ? "Name must be at least 3 characters." :
        !/^[a-zA-Z\s]+$/.test(clean.trim()) ? "Only letters and spaces allowed." : null;
      setErrors(prev => ({ ...prev, name: err }));
      if (value !== clean) {
        Swal.fire({ icon: "warning", title: "Invalid Input", text: "Name can only contain letters and spaces!", timer: 2000, showConfirmButton: false });
      }
      return;
    }
    ...
    if (name === "age") {
      ...
      const err = !num ? "Age is required." :
        parseInt(num, 10) < 18 || parseInt(num, 10) > 99 ? "Age must be 18–99." : null;
    ...
    if (name === "hasShop") {
      setFormData(prev => ({
        ...prev,
        hasShop: value,
        shopAddress: value === "no" ? "" : prev.shopAddress,
        shopFullAddress: value === "no" ? [] : prev.shopFullAddress,
      }));
      setErrors(prev => ({ ...prev, hasShop: value ? undefined : "Please select an option." }));
      return;
    }

    if (name === "aboutUs") {
      if (value.length > 500) {
        Swal.fire({ icon: "warning", title: "Limit Exceeded", text: "Maximum 500 characters allowed.", timer: 2000, showConfirmButton: false });
        return;
      }
      const field = activeTab === "worker" ? "skill" : "aboutUs";
      setFormData(prev => ({ ...prev, [field]: value }));
      const err = !value.trim() ? `${activeTab === "worker" ? "Skill" : "About Us"} is required.` :
        value.trim().length < 10 ? "Minimum 10 characters." : null;
    ...
    geocoder.geocode({ location: { lat, lng } }, (results, status) => {
      if (status === "OK" && results[0]) {
        ...
        if (mapFor === "address") {
          ...
          setTempAddress(prev => ({ ...prev, address, latitude: lat, longitude: lng }));
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          else setMarkerLocationShop(loc);
          getAddressFromLatLng(loc.lat, loc.lng);
          map?.panTo(loc);
        },
        () => Swal.fire({ icon: "error", title: "Location Error", text: "Unable to fetch your location." })
      );
    }
  };
  ...
  const handleMapClick = (e) => {
    ...
    else setMarkerLocationShop({ lat, lng });
    getAddressFromLatLng(lat, lng);
  };
  ...
  const handleSaveAddress = () => {
    if (!tempAddress.title.trim() || !tempAddress.landmark.trim() || !tempAddress.address.trim()) {
      Swal.fire({ icon: "error", title: "Incomplete", text: "Title, Landmark and Address are required." });
      return;
    }
    ...
    if (mapFor === "address") {
      setFormData(prev => ({
        ...
        full_address: [...prev.full_address, { ...tempAddress }],
      }));
      ...
      setFormData(prev => ({
        ...
        shopFullAddress: [{ ...tempAddress, title: "Shop" }],
      }));
      setErrors(prev => ({ ...prev, shopAddress: undefined }));
    }

    setAddressModalOpen(false);
    setTempAddress({ title: "", landmark: "", address: "", latitude: null, longitude: null });
    setIsMapOpen(false);
  };
  ...
  const validateAll = () => {
    ...
    else if (!/^[a-zA-Z\s]+$/.test(formData.name.trim())) errs.name = "Only letters and spaces.";

    if (!formData.age) errs.age = "Age is required.";
    else if (parseInt(formData.age, 10) < 18 || parseInt(formData.age, 10) > 99) errs.age = "Age 18–99.";

    if (!formData.gender) errs.gender = "Gender is required.";
    if (!formData.address.trim()) errs.address = "Address is required.";

    const aboutVal = activeTab === "worker" ? formData.skill : formData.aboutUs;
    if (!aboutVal.trim()) errs.aboutUs = `${activeTab === "worker" ? "Skill" : "About Us"} required.`;
    ...
    if (activeTab === "worker") {
      ...
      if (!formData.emergencysubcategory.length) errs.emergencysubcategory = "Select at least one emergency subcategory.";
      if (!formData.hasShop) errs.hasShop = "Select shop option.";
      if (formData.hasShop === "yes" && !formData.shopAddress.trim()) errs.shopAddress = "Shop address required.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(allErrors).length) {
      setErrors(allErrors);
      Swal.fire({ icon: "error", title: "Validation Error", text: "Please fix all required fields.", confirmButtonColor: "#d33" });
      return;
    }
    ...
    try {
      ...
      if (activeTab === "worker") {
        ...
        if (businessImage.file) fd.append("businessImage", businessImage.file);
        ...
        if (!res.ok) {
          if (res.status === 401) return handleUnauthorized();
          Swal.fire({ icon: "error", title: "Update Failed", text: data.message || "Something went wrong.", confirmButtonColor: "#d33" });
          return;
        }
        ...
        if (profile?.role === "user" || profile?.verificationStatus === "rejected") {
          ...
          if (!upRes.ok) {
            ...
            Swal.fire({ icon: "warning", title: "Role Upgrade", text: upData.message || "Could not send request.", confirmButtonColor: "#f59e0b" });
          } else {
            Swal.fire({ icon: "info", title: "Request Sent", text: "Your role upgrade request has been sent!", confirmButtonColor: "#10b981" });
          }
        }

        dispatch(fetchUserProfile());
        await Swal.fire({ icon: "success", title: "Profile Updated!", text: "Your profile has been updated successfully.", confirmButtonColor: "#22c55e" });
        navigate("/details");
      }
      ...
      console.error(err);
      Swal.fire({ icon: "error", title: "Network Error", text: "Please check your connection.", confirmButtonColor: "#d33" });
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {activeTab === "worker" && (
            <div className="bg-gray-50 p-6 rounded-xl">
              <label className="block mb-4 font-semibold text-gray-700">Business Image (optional)</label>
              <div className="max-w-sm mx-auto space-y-4">
                ...
                {businessImage.preview && (
                  <div className="relative">
                    <img src={businessImage.preview} alt="Business" className="w-full rounded-lg shadow-md" />
                  </div>
                )}
              </div>
            </div>
          ...
          <div  className="ml-6">
            ...
            <input
              ...
              className={`w-full px-4 py-2 rounded-lg border ${errors.name ? "border-red-500" : "border-gray-300"} focus:outline-none focus:ring-2 focus:ring-blue-400 transition`}
            />
          ...
          <div className="ml-6">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="18–99"
              className={`w-full px-4 py-2 rounded-lg border ${errors.age ? "border-red-500" : "border-gray-300"} focus:outline-none focus:ring-2 focus:ring-blue-400 transition`}
            />
          ...
          <div className="ml-6">
            ...
            <select
              ...
              onChange={handleChange}
              className={`w-full px-4 py-2 rounded-lg border ${errors.gender ? "border-red-500" : "border-gray-300"} bg-white focus:outline-none focus:ring-2 focus:ring-blue-400`}
            >
          ...
          {/* <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2 rounded-lg border ${errors.address ? "border-red-500" : "border-gray-300"} cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400 transition`}
            />
            ...
            <>
                ...
                <Select
                  ...
                  onChange={(sel) => {
                    ...
                    setFormData(prev => ({
                      ...
                      emergencysubcategory: id !== prev.category ? [] : prev.emergencysubcategory,
                    }));
                    setErrors(prev => ({ ...prev, category: id ? undefined : "Category required." }));
              ...
              <div className="ml-6">
                ...
                <Select
                  ...
                  onChange={(opts) => {
                    ...
                    setErrors(prev => ({ ...prev, subcategory: ids.length ? undefined : "Select at least one." }));
                  }}
                  isMulti
              ...
              <div className="ml-6">
                ...
                <Select
                  ...
                  onChange={(opts) => {
                    const ids = opts ? opts.map(o => o.value) : [];
                    setFormData(prev => ({ ...prev, emergencysubcategory: ids }));
                    setErrors(prev => ({ ...prev, emergencysubcategory: ids.length ? undefined : "Select at least one." }));
                  }}
                  isMulti
                  isDisabled={!formData.category}
                  placeholder="Search or select..."
                />
                {errors.emergencysubcategory && <p className="text-red-500 text-sm mt-1">{errors.emergencysubcategory}</p>}
              </div>
                ...
                <div className={`bg-gray-50 ml-6 rounded-xl ${errors.selfie ? "ring-2 ring-red-500" : ""}`}>
                  ...
                  <div className="max-w-sm mx-auto space-y-4">
                    ...
                    {documents.selfie.preview && (
                      <div className="relative">
                        <img src={documents.selfie.preview} alt="Selfie" className="w-full max-w-xs mx-auto rounded-lg shadow-md" />
                        <button
                          type="button"
              ...
              <div className="ml-6">
                ...
                <div className="flex items-center space-x-6">
                  <label className="flex items-center">
                    <input type="radio" name="hasShop" value="yes" checked={formData.hasShop === "yes"} onChange={handleChange} className="mr-2" />
                    Yes
                  </label>
                  <label className="flex items-center">
                    <input type="radio" name="hasShop" value="no" checked={formData.hasShop === "no"} onChange={handleChange} className="mr-2" />
                    No
                  </label>
                </div>
                {errors.hasShop && <p className="text-red-500 text-sm mt-2">{errors.hasShop}</p>}
              </div>
              ...
              {formData.hasShop === "yes" && (
                <div className="space-y-3 ml-6">
                  ...
                  <Select
                    ...
                    value={(() => {
                      const cur = formData.shopFullAddress[0];
                      if (!cur?.address) return null;
                      return buildAddressOptions().find(o => o.lat === cur.latitude && o.lng === cur.longitude);
                    })()}
                    onChange={(opt) => {
                      if (!opt) return;
                      setFormData(prev => ({
                        ...prev,
                        shopAddress: opt.label.split(" – ")[1] || opt.label,
                        shopFullAddress: [{
                          title: opt.label.split(" – ")[0] || "Shop",
                          landmark: "",
                          address: opt.label.split(" – ")[1] || opt.label,
                          latitude: opt.lat ?? null,
                          longitude: opt.lng ?? null,
                        }],
                      }));
                      setErrors(prev => ({ ...prev, shopAddress: undefined }));
                    }}
          ...
          <div className="ml-6">
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder={activeTab === "worker" ? "Describe your skill (min 10 chars)..." : "Tell us about yourself (min 10 chars)..."}
              maxLength={500}
              rows={4}
              className={`w-full px-4 py-2 rounded-lg border ${errors.aboutUs ? "border-red-500" : "border-gray-300"} focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none`}
            />
            <p className="text-sm text-gray-500 mt-1">
              {(activeTab === "worker" ? formData.skill : formData.aboutUs).length}/500
            </p>
            {errors.aboutUs && <p className="text-red-500 text-sm mt-1">{errors.aboutUs}</p>}
          </div>
